(function() {
  ...
  function loadPosts(postsList, reverse, searchStr) {
    ...
    postsList.slice(current, current + perPage).forEach(function(item) {
      ...
      let mediaLink = item.external_file || item.local_file;
      let fileExt = mediaLink.split(".").pop();
      ...
      if (fileExt == "mp4") {
        ...
        videoSource.setAttribute('src', mediaLink);
        videoSource.setAttribute('type', `video/${fileExt}`);
        videoObj.append(videoSource);
      }
      else {
        listItemImg = document.createElement("img");
        listItemRealImg = document.createElement("img");
        listItemImg.setAttribute('src', mediaLink);
        listItemImg.classList.add("masonry-content");
        listItemRealImg.setAttribute('data-gifffer', mediaLink);
      ...
      // if (item.external_file) {
        ...
        // listItemRealImg.classList.add("masonry-image");
        downloadButton.setAttribute("data-link", mediaLink);
      ...
      if (fileExt == "mp4") {
        listItemContent.append(videoObj);
      }
      else {
      ...
      imagesLoaded( '.post-list', () => {
        ...
        let newPosts = postsContainer.querySelectorAll('.post-item:not(.initiated)');
        newPosts.forEach(function(post) {
          post.classList.add('initiated');
          let video = post.querySelector('video');
          if (video) {
            addHoverPlayEvents(video);
          }
        });
      });
    });
  ...
  let origPosts = postsContainer.querySelectorAll('.post-item');
  origPosts.forEach(function(post) {
    let video = post.querySelector('video');
    if (video) {
      addHoverPlayEvents(video);
    }
  ...
  function addHoverPlayEvents(video) {
